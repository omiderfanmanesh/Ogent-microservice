#!/bin/bash
# Simplified script to deploy langchain-agent service fixes
...
echo "Creating complete file for agent_service.py..."
cat > agent_service_complete.py << 'EOF'
"""
Agent service.
...
class AgentService:
    ...
    # ... Rest of the AgentService class ...
EOF

# Create temporary file with complete content for conversational_agent.py
echo "Creating complete fixed conversational_agent.py..."
cat > conversational_agent_complete.py << 'EOF'
"""
Fixed version of the run method for the ConversationalAgent class.
This method properly handles tokens_used and correctly creates the AgentResult.
"""
...
# Deploy the files
echo "Copying complete agent_service.py..."
docker cp agent_service_complete.py ogent-agent-service-test:/app/app/core/services/agent_service.py

echo "Copying fixed conversational_agent.py..."
docker cp conversational_agent_complete.py ogent-agent-service-test:/app/app/core/agents/conversational_agent.py

# Clean up temporary files
rm -f agent_service_complete.py
rm -f conversational_agent_complete.py
